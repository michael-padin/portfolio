  ...
  return (
    <div className="min-h-screen pt-28 pb-20">
      <div className="container-main">
        {/* Back */}
        <Link href="/projects" className="inline-flex items-center gap-2 text-text-muted hover:text-accent text-sm mb-10 transition-colors group">
        ...
        {project.techStack?.length > 0 && (
          <div className="card p-6 mb-8">
            <h2 className="text-xs font-mono text-text-muted uppercase tracking-widest mb-4">Tech Stack</h2>
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech) => (
                <span key={tech} className="px-3 py-1.5 rounded-lg bg-accent-subtle text-accent text-sm border border-accent/20 font-mono">
                  {tech}
                </span>
              ))}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          {project.overview && (
            <div className="card p-6">
              <h2 className="text-xs font-mono text-text-muted uppercase tracking-widest mb-3">Overview</h2>
              <p className="text-text-secondary leading-relaxed">{project.overview}</p>
            </div>
          )}
          {project.problem && (
            <div className="card p-6">
              <h2 className="text-xs font-mono text-text-muted uppercase tracking-widest mb-3">The Problem</h2>
        ...
        {project.results && project.results.length > 0 && (
          <div className="card p-6 mb-10">
            <h2 className="text-xs font-mono text-text-muted uppercase tracking-widest mb-6">Results & Impact</h2>
        ...
        <div className="border-t border-surface-border pt-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-text-muted text-sm">I build things like this for clients worldwide.</p>
          </div>
          <Link href="/contact" className="btn-primary shrink-0">Start a project →</Link>
        </div>
      </div>
    </div>
  );
