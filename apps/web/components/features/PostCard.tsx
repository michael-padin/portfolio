...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <Link href={`/blog/${post.slug.current}`} className="card card-accent flex flex-col p-5 gap-4 group">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-[var(--text-muted)] leading-relaxed line-clamp-3">{post.excerpt}</p>
      </div>
